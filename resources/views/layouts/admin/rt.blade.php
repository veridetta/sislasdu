...
<section id="dashboard-analytics">
  @include('panels.flash')
      <div class="col-lg-12 col-sm-12 col-12">
        ...
        aria-controls="DataTables_Table_0" type="button" data-bs-toggle="modal" data-bs-target="#backdrop"><span>Tambah Data RT</span></button>
        @endif
        <div class="card">
          ...
          <div class="card-header bg-warning flex-column align-items-start">
            <div class="row col-12">
                ...
                <div class="col-lg-11 col-10 my-auto">
                    <p class="h4 card-text text-white">Daftar RT</p>
                </div>
            </div>
          </div>
          <div class="card-body" >
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title" id="myModalLabel4">Tambah RT</h4>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <form class="add-new-user pt-0" method="POST" action="{{route('rt-add-admin')}}">
                @csrf
                <input type="hidden" name="id" value="" id="id_rt"/>
                <input type="hidden" name="id_users" value="" id="id_users"/>
                <div class="mb-1">
                  <label class="form-label" for="basic-icon-default-rt">RT</label>
                  <select id="basic-icon-default-rt" class="form-control dt-rt" name="rt" value="{{old('rt')}}">
                ...
                <div class="mb-1">
                  <label class="form-label" for="basic-icon-default-nama">Nama Ketua RT</label>
                  <input
                    type="text"
...
$(function () {
      ...
      var dt_ang = dt_anggota.DataTable({
        ...
        columnDefs: [
          ...
          }@if(auth()->user()->role=='admin'),
          {
            ...
            orderable: false,
            render: function (data, type, full, meta) {
              return '<div class="text-center"><a class="a_edit btn-sm btn btn-primary" pdf="'+full.id_users+'">Ubah</a> <a class=" btn-sm a_delete btn btn-warning" pdf="'+full.id_users+'" href="//{{request()->getHttpHost()}}/admin/d/rt_delete/'+full.id_users+'">Hapus</a></div>';
            }
          }@endif
        ],
  ...
  $('.dt-anggota').on('click', '.a_edit', function () {
        ...
        var url = "//{{request()->getHttpHost()}}/admin/d/rt_data_single/"+dat;
        $.ajax({
          type: "GET",
          url: url,
